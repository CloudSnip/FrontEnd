...
import { classificationRules } from "./utils/calculations";
...
function App() {
  ...
  const [currentWeather, setCurrentWeather] = useState({
    ...
    device: device,
    date: new Date(),
    conditions: classificationRules(temperature, humidity),
  });
  ...
  useEffect(() => {
    setTemperature((2.222).toFixed(1) || 0);
    setHumidity((24.2424).toFixed(1) || 0);
  }, []);
    ...
    const fetchMyDevice = async () => {
      try {
        const response = await API.get(
          "/measurements?filter={name=esp32_9C9D1C}"
        );
        setDevice(response.data.docs[0]);
      } catch (error) {
        ...
        <main className="flex-grow overflow-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 px-4 py-6">
            ...
            <div className="space-y-6">
              ...
              <DailyForecast forecast={dailyForecast} isDarkMode={isDarkMode} />
            </div>
          </div>
        </main>
      </div>
